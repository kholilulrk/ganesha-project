    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm sm:rounded-lg p-6">
                <form x-data="{ loading: false }" method="POST" action="{{ route('announcements.update', $announcement) }}" class="space-y-6">
                    @csrf @method('PATCH')
                    ...
                    <div>
                        ...
                        <textarea id="content" name="content" rows="6" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>{{ old('content', $announcement->content) }}</textarea>
                    ...
                    <div>
                        ...
                        <select id="target_type" name="target_type" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required
                            ...
                            <option value="all_users" {{ old('target_type', $announcement->target_type) == 'all_users' ? 'selected' : '' }}>Semua User</option>
                            <option value="specific" {{ old('target_type', $announcement->target_type) == 'specific' ? 'selected' : '' }}>User / Role Tertentu</option>
                        </select>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <x-text-input id="scheduled_date" class="block mt-1 w-full" type="date" name="scheduled_date" :value="old('scheduled_date', $announcement->scheduled_at?->format('Y-m-d'))" />
                            <x-input-error :messages="$errors->get('scheduled_date')" class="mt-2" />
                        </div>
                        <div>
                            <x-input-label for="scheduled_time" :value="__('Jam Mulai')" />
                            <x-text-input id="scheduled_time" class="block mt-1 w-full" type="time" name="scheduled_time" :value="old('scheduled_time', $announcement->scheduled_at?->format('H:i') ?? '08:00')" />
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <x-text-input id="expired_date" class="block mt-1 w-full" type="date" name="expired_date" :value="old('expired_date', $announcement->expired_at?->format('Y-m-d'))" />
                            <x-input-error :messages="$errors->get('expired_date')" class="mt-2" />
                        </div>
                        <div>
                            <x-input-label for="expired_time" :value="__('Jam Berakhir')" />
                            <x-text-input id="expired_time" class="block mt-1 w-full" type="time" name="expired_time" :value="old('expired_time', $announcement->expired_at?->format('H:i') ?? '17:00')" />
                    ...
                    <div id="target_specific" style="display: {{ old('target_type', $announcement->target_type) == 'specific' ? 'block' : 'none' }}">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <select id="target_users" name="target_users[]" multiple class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" size="6">
                                    @foreach ($users as $u)
                                        <option value="{{ $u->id }}" {{ in_array($u->id, old('target_users', $announcement->specificUsers->pluck('id')->toArray())) ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <select id="target_roles" name="target_roles[]" multiple class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" size="6">
                                    @foreach ($roles as $r)
                                        <option value="{{ $r->id }}" {{ in_array($r->id, old('target_roles', $announcement->specificRoles->pluck('id')->toArray())) ? 'selected' : '' }}>
                                            {{ $r->name }}
                                        </option>
                                    @endforeach
                                </select>
