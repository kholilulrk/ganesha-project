<x-guest-layout>
    <form x-data="{ loading: false }" x-on:submit="loading = true" method="POST" action="{{ route('register') }}">
        @csrf
        ...
        <div class="mt-4">
            ...
            <select id="role" name="role" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                ...
                <option value="logistic" {{ old('role') == 'logistic' ? 'selected' : '' }}>Logistic / Purchasing</option>
            </select>
