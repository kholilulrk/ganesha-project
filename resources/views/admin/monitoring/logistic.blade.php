<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight break-words">{{ __('Monitoring Logistic') }}</h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-4">
                    <p class="text-sm text-gray-500">Total Task (dengan Shopping List)</p>
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-4 border-l-4 border-green-400">
                    <p class="text-sm text-gray-500">Selesai (Shopping Tercheck Semua)</p>
                ...
                <div class="p-6">
                    ...
                    <table class="min-w-full w-full text-sm whitespace-nowrap">
                        ...
                        <tbody>
                                ...
                                <tr class="border-b last:border-0">
                                    ...
                                    <td class="py-3">
                                        <div class="flex items-center gap-2">
                                            <span class="text-xs {{ $task->checked_count === $task->shopping_items_count ? 'text-green-600 font-medium' : 'text-yellow-600' }}">
                                                {{ $task->checked_count }}/{{ $task->shopping_items_count }}
                                            </span>
                                            <div class="w-16 bg-gray-200 rounded-full h-1.5">
                                                <div class="h-1.5 rounded-full {{ $task->checked_count === $task->shopping_items_count ? 'bg-green-500' : 'bg-yellow-500' }}" style="width: {{ $task->shopping_items_count > 0 ? ($task->checked_count / $task->shopping_items_count) * 100 : 0 }}%"></div>
                                            </div>
                                        </div>
                                    </td>
                            ...
                            @empty
                                <tr><td colspan="6" class="py-6 text-center text-gray-500">No logistic tasks with shopping list found.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                    </div>
